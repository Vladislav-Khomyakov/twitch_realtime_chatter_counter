...
- 🤖 **Исключение ботов** - автоматически исключает чат-ботов из подсчета участников
- 📋 **Экспорт списков** - возможность экспортировать списки пользователей и ботов
- 🗂️ **Поддержка нескольких вкладок** - работает одновременно на нескольких вкладках Twitch
- 📈 **Глобальная статистика** - показывает общее количество пользователей со всех вкладок
- 🔄 **Переключение между вкладками** - удобное переключение между активными вкладками Twitch
...
4. **Работа с ботами**
   - **Получить список ботов** - нажмите кнопку "Получить ботов" в popup
   - **Важно**: Для получения списка ботов необходимо открыть вкладку "Сообщество" в чате Twitch
   - Боты автоматически исключаются из подсчета участников
   - Список ботов сохраняется между сессиями

5. **Работа с несколькими вкладками**
   - Расширение автоматически обнаруживает все открытые вкладки Twitch
   - В popup отображается список всех активных вкладок Twitch
   - Клик по вкладке переключает на неё и показывает её статистику
   - Глобальная статистика показывает общее количество пользователей со всех вкладок
   - Каждая вкладка ведет независимый подсчет пользователей

6. **Экспорт данных**
   - **Экспорт пользователей** - копирует список всех уникальных пользователей в буфер обмена
   - **Экспорт ботов** - копирует список найденных ботов в буфер обмена
...
### Проблемы с получением списка ботов

1. **Откройте вкладку "Сообщество" в чате**
   - В чате Twitch найдите и нажмите на вкладку "Сообщество"
   - Убедитесь, что вкладка полностью загрузилась

2. **Проверьте наличие ботов**
   - В разделе "Сообщество" должен быть блок "Чат-боты"
   - Если ботов нет, список будет пустым

3. **Повторите попытку**
   - Нажмите кнопку "Получить ботов" в popup расширения
   - При необходимости обновите страницу и повторите процедуру
